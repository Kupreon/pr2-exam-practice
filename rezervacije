...
public:
    ...
    void AddElement(T1 el1, T2 el2) {
        ...
        _elementi1[*_trenutno - 1] = el1;
        _elementi2[*_trenutno - 1] = el2;
    }
    ...
    Kolekcija& InsertAt(int lokacija, T1 el1, T2 el2) {
        ...
        _elementi1[*_trenutno - 1] = _elementi1[*_trenutno - 2];
        _elementi2[*_trenutno - 1] = _elementi2[*_trenutno - 2];

        for (int i = *_trenutno - 2; i > lokacija; i--) {
...
public:
    ...
    void AddOcjenuKriterija(Kriteriji krit, int ocjena) {
        for (int i = 0; i < _ocjeneKriterija->getTrenutno(); i++)
            if (krit == _ocjeneKriterija->getElement1(i))
                throw exception("\nKriterij vec ocjenjen!\n");

        _ocjeneKriterija->AddElement(krit, ocjena);
    }
    ...
    friend ostream& operator<< (ostream& COUT, const Komentar& obj) {
        ...
        for (int i = 0; i < obj._ocjeneKriterija->getTrenutno(); i++) {
            prosjek += obj._ocjeneKriterija->getElement2(i);
            cout << '\t' << getKriterij(obj._ocjeneKriterija->getElement1(i)) << "(" << obj._ocjeneKriterija->getElement2(i) << ")" << endl;
        }

        prosjek /= obj._ocjeneKriterija->getTrenutno();
        cout << crt << "Prosjecna ocjena ->   " << prosjek;

        return COUT;
    }

    static string getKriterij(Kriteriji krit) {
        switch (krit) {
        case 0: return "CISTOCA"; break;
        case 1: return "USLUGA"; break;
        case 2: return "LOKACIJA"; break;
        case 3: return "UDOBNOST"; break;
        }
    }
};
...
bool ValidirajBrojPasosa(string brojPasosa) {
    string pravilo = "([A-Z]{1,2}\\d{3,4}[- ]?\\d{2,4})";

    regex regexPravilo(pravilo);

    return regex_search(brojPasosa, regexPravilo);
}
...
class Gost {
    ...
    unique_ptr<char[]> Alociraj(const char* sadrzaj) {
        if (sadrzaj == nullptr) return nullptr;
...
public:
    Gost(const char* imePrezime, string emailAdresa, string brojPasosa) : _imePrezime(Alociraj(imePrezime)) {
        //_imePrezime = "INICIJALIZOVATI...";
        _emailAdresa = emailAdresa;
        if (ValidirajBrojPasosa(brojPasosa) == true) { _brojPasosa = brojPasosa; }
        else { _brojPasosa = "NOT VALID"; }
    }
    Gost(const Gost& obj) : _imePrezime(Alociraj(obj._imePrezime.get())) {
        _emailAdresa = obj._emailAdresa;
        if (ValidirajBrojPasosa(obj._brojPasosa) == true) { _brojPasosa = obj._brojPasosa; }
        else { _brojPasosa = "NOT VALID"; }
    }
...
bool operator==(Gost& gost1, Gost& gost2) {
    if (strcmp(gost1.GetImePrezime(), gost2.GetImePrezime()))
        return false;
...
public:
    ...
    Rezervacija(const Rezervacija& obj) {
        ...
        for (int i = 0; i < obj._gosti.size(); i++) {
            _gosti.push_back(obj._gosti.at(i));
        }
    ...
    void SetKomentar(const Komentar& komentar) {
        ...
        if (ocjenaCounter >= 2)
        {
            thread radnik(&Rezervacija::SaljiEmail, *this);
            radnik.join();
        }
    }
...
const char* GetOdgovorNaPrvoPitanje() {
    ...
    return "Odgovor -> Polimorfizam je prisutan kada imamo nasledjivanje gdje izvedena klasa override-a neku baznu metodu koja je oznacena kao virtual.";
}
const char* GetOdgovorNaDrugoPitanje() {
    ...
    return "Odgovor -> Abstract klasa se ne moze instancirati i ona predstavlja interfejs za izvedene klase koje moraju sadrzati metode koje su u baznoj klasi.";
}
